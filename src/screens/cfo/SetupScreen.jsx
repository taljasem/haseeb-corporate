...
import {
  ...
  UserMinus, Banknote, Briefcase, Users2,
} from "lucide-react";
...
import {
  ...
  // FN-272 KYC admin — vendor + customer CRUD wrappers. Backend live at
  // corporate-api 493030e (HASEEB-143).
  listVendors,
  deactivateVendor,
  listCustomers,
  deactivateCustomer,
...
import KYCEditModal from "../../components/setup/KYCEditModal";
...
const ALL_SECTIONS = [
  ...
  // FN-272: KYC admin (CR-number / expiry / civil-ID / KYC notes) for
  // vendors + customers. Per Phase 4 sidebar-crowding flag, SetupScreen
  // is now at 16 sub-sections (was 14). If user feedback shows the
  // sidebar feels crowded, Phase 5 should extract these two into a
  // dedicated "Contacts" screen — not blocking this ship.
  { id: "vendors",         icon: Briefcase,      foreignOnly: false },
  { id: "customers",       icon: Users2,         foreignOnly: false },
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          {SECTIONS.map((s) => {
            ...
            return (
              ...
              >
                ...
                <span>{t(`sections.${s.id === "chart" ? "chart_of_accounts" : s.id === "fiscal" ? "fiscal_year" : s.id === "currencies" ? "currencies" : s.id === "integrations" ? "integrations" : s.id === "team_access" ? "team_access" : s.id === "engine_rules" ? "engine_rules" : s.id === "disallowance" ? "disallowance" : s.id === "tax_lodgement" ? "tax_lodgement" : s.id === "cit_assessment" ? "cit_assessment" : s.id === "wht" ? "wht" : s.id === "cost_allocation" ? "cost_allocation" : s.id === "related_party" ? "related_party" : s.id === "vendors" ? "vendors" : s.id === "customers" ? "customers" : s.id === "warranty" ? "warranty" : s.id === "bank_formats" ? "bank_formats" : s.id === "leave" ? "leave" : s.id === "cbk_rates" ? "cbk_rates" : s.id}`)}</span>
              </button>
            );
          })}
        </aside>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "22px 28px 32px", minWidth: 0 }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            {active === "vendors"       && <PartyKycSection kind="vendor" readOnly={readOnly} />}
            {active === "customers"     && <PartyKycSection kind="customer" readOnly={readOnly} />}
...
// ── Vendors / Customers KYC admin (FN-272, 2026-04-19) ────────────
//
// Single shared section with a `kind` prop ('vendor' | 'customer') that
// renders the master list of the relevant party type with search +
// expiry warnings + a row-level Edit button that opens KYCEditModal.
// The detail drawer surfaces all KYC fields read-only (with the same
// expiry chip pattern as the row); Edit transitions through the
// shared modal.
//
// Backend endpoints (corporate-api 493030e, HASEEB-143):
//   GET   /api/{vendors|customers}        — list + search + pagination
//   GET   /api/{vendors|customers}/:id    — detail
//   POST  /api/{vendors|customers}        — create (OWNER | ACCOUNTANT)
//   PATCH /api/{vendors|customers}/:id    — partial update (OWNER | ACCOUNTANT)
//   DELETE /api/{vendors|customers}/:id   — soft-delete (OWNER only)
//
// Role gating: SetupScreen's `readOnly` prop already prevents non-CFO/
// non-Senior roles from opening the section's edit affordances. Backend
// is authoritative on writes. Deactivate is hidden when readOnly to
// match the surrounding pattern; the backend's OWNER-only check on
// DELETE will surface as a 403 toast for non-Owner CFOs anyway.
//
// Expiry warning chip pattern:
//   crExpiryDate <= today              → "Expired"          (danger)
//   crExpiryDate within +30 days       → "Expiring in N days" (warning)
//   crExpiryDate > today + 30 days     → no chip
//   crExpiryDate is null               → "No CR tracked"    (neutral)
...
function PartyKycSection({ kind = "vendor", readOnly = false }) {
  const { t } = useTranslation("setup");
